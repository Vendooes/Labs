...
def find_similar_word(text, target_word, d):
    """Находит первое похожее слово в тексте с учетом ограничений на операции."""
    words = text.split()
    best_index = -1
    ...
    for index, word in enumerate(words):
        ...
        if distance <= d:
            if (best_index == -1) or (len(word) < best_length):
                best_index = index
...
result = find_similar_word(text, target_word, d)
if result:
